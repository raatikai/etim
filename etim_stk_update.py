...
def handler() -> bool:
    try:
        ...
        for supplier in suppliers:
            ...
            duplicate_keys = get_duplicate_key_pairs(inriver_items_json, 'ItemElectricityNumber')
            if duplicate_keys:
                logger.error(f'Supplier {supplierVat} has {len(duplicate_keys)} duplicate ItemElectricityNumber+ItemSupplierNumber keys in Inriver; ambiguous items will be skipped.')
                logger.error(f'Duplicate keys sample: {list(duplicate_keys)[:10]}')
                inriver_items_json = filter_out_duplicate_key_rows(inriver_items_json, 'ItemElectricityNumber', duplicate_keys)
